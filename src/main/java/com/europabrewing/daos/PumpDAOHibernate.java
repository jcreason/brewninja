 ...
 */
public class PumpDAOHibernate implements PumpDAO {


	private final static Logger logger = LogManager.getLogger(PumpDAOHibernate.class.getName());

	private final Session session;

	public PumpDAOHibernate(Session session) {
		this.session = session;
	}

	@Override
	public List<Pump> getEnabledPumps() {
		return HibernateUtil.listAndCast(session.createQuery("from Pump where disabled = false "));
	}
}
